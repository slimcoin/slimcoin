...
inline int OutputDebugStringF(const char* pszFormat, ...)
{
  ...
  //write to the debug file regardless
  {
    // print to debug.log
    if(!fileout)
    {
...
#ifdef WIN32
  // Windows
  return MyGetSpecialFolderPath(CSIDL_APPDATA, true) / "SLIMCoin";
#else
  fs::path pathRet;
...
boost::filesystem::path GetConfigFile()
{
  ...
  fs::path pathConfigFile(GetArg("-conf", "slimcoin.conf"));
  ...
  if(vTimeOffsets.size() >= 5 && vTimeOffsets.size() % 2 == 1)
  {
    ...
    else
    {
      ...
      if(!fDone)
      {
        ...
        {
          fDone = true;
          string strMessage = _("Warning: Please check that your computer's date and time are correct.  If your clock is wrong SLIMCoin will not work properly.");
          strMiscWarning = strMessage;
          printf("*** %s\n", strMessage.c_str());
          ThreadSafeMessageBox(strMessage+" ", string("SLIMCoin"), wxOK | wxICON_EXCLAMATION);
        }
      }
    }
    if(fDebug) {
...
{
  return MyGetSpecialFolderPath(CSIDL_STARTUP, true) / "SLIMCoin.lnk";
}
...
{
  return GetAutostartDir() / "slimcoin.desktop";
}

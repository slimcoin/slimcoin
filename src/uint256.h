...
template<typename T>
inline T uint256_get_top(uint256 phash)
{
  /* 32 / sizeof(T) gets the number of bytes the data of uint256 is held in
   * subtract 1 from that to get the last element, in this case it is the first
   * since the hash is reversed */
  return ((T*)&phash)[(32 / sizeof(T)) - 1];
}

...
// slimcoin: optional setting to unlock wallet for block minting only;
...
// slimcoin: total coins staked (non-spendable until maturity)
...
bool CWallet::SelectCoinsMinConf(int64 nTargetValue, unsigned int nSpendTime, int nConfMine, int nConfTheirs,
                                 set<pair<const CWalletTx*,unsigned int> >& setCoinsRet, int64& nValueRet) const
{
    ...
    BOOST_FOREACH(const CWalletTx* pcoin, vCoins)
    {
      ...
      for(unsigned int i = 0; i < pcoin->vout.size(); i++)
      {
        ...
        if(pcoin->nTime > nSpendTime)
          continue;  // slimcoin: timestamp must not exceed spend time
        ...
        //if the value of that transaction value is exactly the target value, use it and exit
        if(n == nTargetValue)
        {
          setCoinsRet.insert(coin.second);
          nValueRet += coin.first;
          return true;
        }else if(n < nTargetValue + CENT) //else, add the coin to the vector and increase the the value counter
        {
  ...
  //if the stake value counter is equal to the target
  if(nTotalLower == nTargetValue || nTotalLower == nTargetValue + CENT)
  {
    //for every value, add it to setCoinsRet and incrememnt nValueRet by its value
  ...
  //if the stake counter is less than the target value
  if(nTotalLower < nTargetValue + (coinLowestLarger.second.first ? CENT : 0))
  {
    //make sure the CWalletTx is not NULL
...
bool CWallet::SelectCoins(int64 nTargetValue, unsigned int nSpendTime, 
                          set<pair<const CWalletTx*,unsigned int> >& setCoinsRet, int64& nValueRet) const
{
...

  {
    ...
    CTxDB txdb("r");
    {
      ...
      for(;;)
      {
        ...
        // slimcoin: sub-cent change is moved to fee
        ...
        if(nChange > 0)
        {
          ...
          if(!GetBoolArg("-avatar")) // slimcoin: not avatar mode
          {
                              ...
                              int64 nSearchInterval, CTransaction &txNew)
{
  ...
  BOOST_FOREACH(PAIRTYPE(const CWalletTx*, unsigned int) pcoin, setCoins)
  {
    ...
    // Read block index
    if(!mapBlockIndex.count(pcoin.first->hashBlock)) //sanity check
      continue;
    CBlockIndex *pindex = mapBlockIndex[pcoin.first->hashBlock];

    static int nMaxStakeSearchInterval = 60;
    if(pindex->GetBlockTime() + nStakeMinAge > txNew.nTime - nMaxStakeSearchInterval)
      continue; // only count coins meeting min age requirement

    bool fKernelFound = false;
    for(unsigned int n = 0; n < min(nSearchInterval, (int64)nMaxStakeSearchInterval) && 
          !fKernelFound && !fShutdown; n++)
    {
      // Search backward in time from the given txNew timestamp 
      // Search nSearchInterval seconds back up to nMaxStakeSearchInterval
      uint256 hashProofOfStake = 0;
      COutPoint prevoutStake = COutPoint(pcoin.first->GetHash(), pcoin.second);
      if(CheckStakeKernelHash(nBits, pindex, txindex.pos.nTxPos - txindex.pos.nBlockPos,
                              *pcoin.first, prevoutStake, txNew.nTime - n, hashProofOfStake))
      {
        ...
        if(pindex->GetBlockTime() + nStakeSplitAge > txNew.nTime)
  ...
  BOOST_FOREACH(PAIRTYPE(const CWalletTx*, unsigned int) pcoin, setCoins)
  {
    ...
    if(txNew.vout.size() == 2 && ((pcoin.first->vout[pcoin.second].scriptPubKey == scriptPubKeyKernel || 
...
// slimcoin: check 'spent' consistency between wallet and txindex
// slimcoin: fix wallet spent state according to txindex
...
// slimcoin: disable transaction (only for coinstake)
